...
class Mmc(object):
    ...
    def __init__ (self,cluster,trailMobilityTrace,userid,
        ...
        """self._spatiaTemporallLabeledTrailmt: This list contains the spatio (poi) 
        and temporal (time windows) labels to compute the transition matrix.
        """
        self._timeTrailmt=[]
        """self._timeTrailmt: This list contains the time of the mt  labels to 
        compute the time of transition matrix as well as the variance of time 
        ...
        self._timeTransitionMatrix = numpy.zeros(shape=(size,size))   #Time transition  matrix
        """self._timeTransitionMatrix : This matrix contains the average number of seconds spent
           to go from a POI A to a POI B
        """
        #Variance Time transition  matrix
        self._varianceTimeTransitionMatrix = numpy.zeros(shape=(size,size))
        """self._varianceTimeTransitionMatrix : This matrix contains the variance of 
           the  number of seconds spent to go from a POI A to a POI B
        """
...
########################################
    def buildModel (self,local=True,pBuildTrajectories=False,pSquash=True):
        logging.info("Building mmc...")
        logging.info("Creating labels")
        self.__prelabelMobilityTraces__(buildTrajectories=pBuildTrajectories,squash=pSquash)
        ...
        logging.info("Building transition matriw")
...
########################################
    ...
    def printTrajectories(self):
        ...
        """
        print "idTrajectory,latitude,longitude,timestamp\n"
        trajectory = 0
        if len(self._dict_mt_trajectory) != 0:
            for key in self._dict_mt_trajectory:
                #key shows the id of the POIs
                print key
    ...
    def __prelabelMobilityTraces__(self,buildTrajectories=True,squash=True):
    ...
    def __labelMobilityTrace__ (self):
        ...
        if (self._order == 2):
            ...
            while (i < size):
                ...
                timeMobilityTraces.append(mt.timestamp)
        ...
        self._timeTrailmt = timeMobilityTraces

    #end __labelMobilityTrace__

    def labelMobilityTrace (self,pTrailMobilityTrace):

        """This method label each mobility trace in the same format as the spatiotemporal
           labels to use it for the transition matrix
        """
        #Stock attriibutes 
        auxTrailMT = self.trailMobilityTrace
        auxSpatialLabelRaw = self._spatialLabelRaw

        #Generate new spatial labels
        self.trailMobilityTrace = pTrailMobilityTrace
        self.__prelabelMobilityTraces__(buildTrajectories = True)

        labeledMobilityTraces = []
        size = len (self._spatialLabelRaw)
        print self._spatialLabelRaw
        ...
        self.trailMobilityTrace = auxTrailMT
        self._spatialLabelRaw = auxSpatialLabelRaw

        return labeledMobilityTraces
...
#########################################
    ...
    def computedTransitionMatrix (self, local=True):
        ...
        while (i < size):
            ...
            #computes the time to go from one POI to another
            time_i = self._timeTrailmt[i]
            time_j = self._timeTrailmt[i+1]
            elapsedTime = abs(time_j-time_i).total_seconds()

            self._timeTransitionMatrix[index_i][index_j] += elapsedTime
            self._varianceTimeTransitionMatrix[index_i][index_j] += (elapsedTime*elapsedTime)
            i += 1

        if (local):
            index = len(self._spatioTemporalLabels)-1
            #Erases rows and columns without values
            ...
            while (index >= 0):
                if (self._countMatrix[index].sum() == 0):
                    ...
                    self._timeTransitionMatrix = numpy.delete(
                            self._timeTransitionMatrix, 
                            (index), 
                            axis=0) 
                    self._varianceTimeTransitionMatrix = numpy.delete(
                            self._varianceTimeTransitionMatrix, 
                            (index),
                            axis=1)
                    del self._pois[index]
        ...
        #print "transition: \n {0}".format(self._transitionMatrix)
        #Normalize time  matrices

        (rows,cols) = self._countMatrix.shape
        row = 0
        col = 0
        ...
        while row < rows:
           col = 0
           while col < cols:
               auxTime = self._timeTransitionMatrix[row][col]
               auxVarTime = self._varianceTimeTransitionMatrix[row][col]
               count = self._countMatrix[row][col]
               if ((auxTime > 0) & (count > 0)):
                   self._timeTransitionMatrix[row][col] = float(auxTime/count)
               if ((auxVarTime > 0) & (count > 0)):
                   self._varianceTimeTransitionMatrix[row][col] = float(auxVarTime/count)
               col += 1
...
#################################################
    ...
    def prediction (self, indexCurrentState):
        """ This method predict the next whereabouts of the user's model
            based on the maximal output trajectory.
            @indexCurrentState is the index of the self._spatioTemporalLabels representing
            the current state of the MMC
            Returns:
            @State: the predicted state in form of a list
            @timeTransition: the average time to go from current to predicted POI
            @varianceTimeTransition: the variance of the  time to go from current 
                to predicted POI
        """
        state = -3
        timeTrans = -3
        ...
        if indexCurrentState < len(self._spatioTemporalLabels):
            currentState = self._spatioTemporalLabels[indexCurrentState]

        if currentState in self._spatioTemporalLabels:
            index_currentState = self._spatioTemporalLabels.index(currentState)
            selected_row = self._transitionMatrix[index_currentState]
            index_prediction = Mmc.__indexMaxValue__(selected_row)
            #print "predicted state: {0}".format(self._spatioTemporalLabels[index_prediction])
            
            timeTrans = self._timeTransitionMatrix[index_currentState][index_prediction]
            varTimeTrans = self._varianceTimeTransitionMatrix[index_currentState][index_prediction]
            if index_prediction != -1:
               state = self._spatioTemporalLabels[index_prediction]
            else:
               state = -1
               timeTransition = -1
               varianceTimeTransition = -1

        else:
            state = -2
            timeTransition = -2
            varianceTimeTransition = -2

        return (state,timeTrans,varTimeTrans)
    #end prediction

    @classmethod
    def __indexMaxValue__(cls,transitionVector):
        """
        """
        indexVector = list()
        valueMax = -1
        ...
        if len(transitionVector) > 0:
            for t in transitionVector:
                if t > valueMax:
                    valueMax = t
            #if there are ties in the max value
            index = 0
            for t in transitionVector:
                if t == valueMax:
            ...
            if len(indexVector) == 1:
                ...
                index = indexVector[randint(0,(len(indexVector)-1)) ]

        return index
    #end __indexMaxValue__
...
################################################
    ...
    def exportToMap(self, path):
        ...
        with open(path, 'wb') as csvfile:
            ...
            oWriter.writerow(["poi","latitude","longitude"])
...
######################################
    ...
    def getClosestPOI(self,lat,lon):
        """Method computes the closest poi of a MMC to a given latitude
        and longitud
        @closest_poi Returns the KEY of the "self._pois" dictionary corresponding to the closest state 
        @auxDistance returns the distance between the medioid of the closest state and the lat lon
        @index_closest_poi return the index as ineteger corresponding to the list of spatio temporal
        labels "self._spatioTemporalLabels"
        """
        pois = self._pois
        closest_poi = None
        auxDistance = sys.float_info.max
        i = 0
        closest_poi_index = -1
        for key in pois:
            medioid = pois[key][1]
            if medioid.distance_latlon(lat,lon) < auxDistance:
                auxDistance = medioid.distance_latlon(lat,lon)
                closest_poi = key 
                closest_poi_index = i
            i += 1

        return (closest_poi_index,closest_poi,auxDistance)
    #end getClosestPOI


    def getClosestPOIwithout(self,lat,lon,index):
        """Method computes the closest poi of a MMC to a given latitude
        and longitud expluding the value in the @index. W.r.t., it returns the cloisest poi withou taking 
        into account the poi given by the @index
        @index: index of the poi we want to exclude
        @closest_poi Returns the KEY of the "self._pois" dictionary corresponding to the closest state 
        @auxDistance returns the distance between the medioid of the closest state and the lat lon
        @index_closest_poi return the index as ineteger corresponding to the list of spatio temporal
        labels "self._spatioTemporalLabels"
        ...
        for key in pois:
            medioid = pois[key][1]
            if ((medioid.distance_latlon(lat,lon) < auxDistance) and index != i):
                auxDistance = medioid.distance_latlon(lat,lon)
                closest_poi = key 
                closest_poi_index = i
            i += 1
    ...
    def getClosestPOI_mt(self,mt):
        """Method computes the closest poi of a MMC to a given latitude
        and longitud
        Returns the closest state and the distance
        """
        return getClosestPOI(self,mt.latitude,mt.longitude)
    #end getClosestPOI_mt

    def getPOIorderedByStationaryValue(self):
        """Return sorted dictionary where key is the order based on the value. Where the first element correspond 
        to the lowest value of the stationary 
        """
        sortedDict = dict()
        sorted_index = [i[0] for i in sorted(enumerate(self._cumulatedStationary), key=lambda x:x[1])]
        i = 0
        for key in self._pois:
            sortedDict[i] = self._pois[key]
            i += 1
        return sortedDict

    #end getPOIorderedByStationaryValue
