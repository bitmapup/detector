##!/usr/bin/env python
"""
Contains the MobiliTrace class and the methods to interact with it.
"""
import copy
import math
import numpy
...
#################################################
    ...
    def distance(self, mobilityTrace):
        ...
        """ Computes the Harvesian distance  in Km"""
    ...
    def time_distance(self, mobilityTrace):
        ...
        deltaTime = abs(mobilityTrace.timestamp - self.timestamp)
    ...
    def speed (self, mobilityTrace):
        ...
        if (deltaTime.total_seconds() != 0):
            computedSpeed = float( self.distance(mobilityTrace)/(deltaTime.total_seconds()/3600))
        else:
            computedSpeed = 0
...
#################################################
    ...
    @classmethod
    def computeFarthestPointFromMedioid(cls,trialMobilityTraces):
        """ Computes the  the distance between the 
            farthest mobility trace and the medioid"""
        size=len(trialMobilityTraces)
        matrix = [[0]*size for i in range(size)]
        cumulatedDistance = [0]*size
        i = 0
        ...
        minValue = min(cumulatedDistance)
        medioid =  trialMobilityTraces[cumulatedDistance.index(minValue)]
        maxValue = max(cumulatedDistance)
        farthest = trialMobilityTraces[cumulatedDistance.index(maxValue)]
        return medioid.distance(farthest)
    #end computeMaxPoint
